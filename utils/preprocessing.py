...
def construir_split(
    ...
    """
    Construye los conjuntos de entrenamiento, validación y prueba a partir de
    un diccionario de olas por conjunto.
    ...
    split        : dict con las olas de 'train', 'val' y 'test'. Por defecto el
                   corte definitivo SPLIT; los pliegues históricos de
                   SPLITS_TEMPORALES se pasan por este parámetro para que las
                   reglas de exclusión sean idénticas en todos los cortes.
...
def pesos_muestrales(df_sub: pd.DataFrame, normalizar: bool = True) -> np.ndarray:
    """
    Factor de expansión muestral del Latinobarómetro (`X_020`) por registro.

    A diferencia de los pesos de entrenamiento que construye `construir_split`,
    aquí NO se multiplica por el peso de clase: los pesos de clase corrigen el
    desbalance durante el ajuste, mientras que este factor corrige el diseño
    muestral y es el único que corresponde usar al reportar métricas
    representativas de la muestra nacional.

    Parámetros
    ----------
    df_sub     : subconjunto de datos que contiene la columna COL_PESO.
    normalizar : divide por la media para que el peso medio sea 1, de modo que
                 las métricas ponderadas y no ponderadas sean comparables en
                 escala.

    Retorna
    -------
    Array de pesos. Si la columna no existe, devuelve un vector de unos.

    Nota sobre el estimando: `X_020` corrige el diseño DENTRO de cada país.
    Como Latinobarómetro entrevista un número similar de personas en cada país
    con independencia de su población, las métricas agregadas ponderadas por
    `X_020` siguen dando peso equivalente a cada país; una lectura poblacional
    de la región exigiría además reescalar por población nacional.
    """
    if COL_PESO not in df_sub.columns:
        return np.ones(len(df_sub), dtype=float)
    w = pd.to_numeric(df_sub[COL_PESO], errors="coerce").fillna(1.0).astype(float)
    w = w.mask(w <= 0, 1.0)
    if normalizar and w.mean() > 0:
        w = w / w.mean()
    return w.to_numpy()


def resumen_split(X_tr, y_tr, X_val, y_val, X_te, y_te, titulo="Split único"):
    print(f"{'─'*52}")
    print(f"  {titulo}")
    print(f"{'─'*52}")
...
def predecir_conjunto(art: dict, df_split: pd.DataFrame) -> Tuple:
    """
    Reconstruye las predicciones de un pipeline entrenado sobre un conjunto
    completo leído de Parquet.

    Aplica exactamente la misma cadena de preprocesamiento que se usó al
    entrenar, según la familia del modelo:

    - **OLO y TabNet** reciben la matriz imputada y escalada con los objetos
      ajustados sobre el conjunto de entrenamiento y guardados en el artefacto.
    - **Árboles de gradiente** reciben los valores ausentes sin imputar —los
      tratan de forma nativa— con las variables categóricas en la codificación
      que exige cada librería (ver `preparar_features_modelo`).

    Parámetros
    ----------
    art      : artefacto del pipeline (`cargar_pipeline`).
    df_split : conjunto leído con `cargar_split_parquet`; puede traer columnas
               de metadatos (target, país, año, peso) que se ignoran.

    Retorna
    -------
    (y_pred, y_prob) con `y_pred` de enteros y `y_prob` en `None` si el modelo
    no expone `predict_proba`.
    """
    feats = art["features"]
    faltantes = [f for f in feats if f not in df_split.columns]
    if faltantes:
        raise ValueError(
            f"predecir_conjunto(): faltan {len(faltantes)} features en el "
            f"conjunto: {faltantes[:8]}"
        )
    X = df_split[feats]
    tipo = art.get("tipo_modelo", "trees")

    if tipo in ("olo", "tabnet"):
        vars_cat = [c for c in art.get("vars_categoricas", []) if c in feats]
        cols_num = [c for c in feats if c not in vars_cat]
        X_imp = pd.DataFrame(art["imp_num"].transform(X[cols_num]),
                             columns=cols_num, index=X.index)
        if vars_cat and art.get("imp_cat") is not None:
            X_cat = pd.DataFrame(art["imp_cat"].transform(X[vars_cat]),
                                 columns=vars_cat, index=X.index)
            X_imp = pd.concat([X_imp, X_cat], axis=1)[feats]
        X_ent = X_imp.copy()
        X_ent[cols_num] = art["scaler"].transform(X_imp[cols_num])
        X_ent = X_ent.values
        if tipo == "tabnet":
            X_ent = X_ent.astype(np.float32)
    else:
        X_ent = preparar_features_modelo(X, art)

    y_pred = np.ravel(art["modelo"].predict(X_ent)).astype(int)
    try:
        y_prob = np.asarray(art["modelo"].predict_proba(X_ent))
